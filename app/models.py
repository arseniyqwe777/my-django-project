...
def book_cover_upload_path(instance, filename):
    """
    Детерминированное имя файла обложки: covers/<inventory_number>.<ext>
    Если инвентарный номер ещё не задан (новая книга без сохранения),
    используем короткий уникальный суффикс вместо случайного имени Django.
    Повторная загрузка обложки для той же книги перезапишет старый файл
    вместо создания дубликата с рандомным суффиксом (_BbJafiX и т.п.).
    """
    ext = os.path.splitext(filename)[1].lower()
    if instance.inventory_number:
        base_name = instance.inventory_number
    else:
        base_name = uuid.uuid4().hex[:8]
    return f"covers/{base_name}{ext}"
...
class Book(models.Model):
    ...
    inventory_number = models.CharField(
        ...
        max_length=10,
        unique=True,
        help_text="Инвентарный номер (например: 00001, 12345)"
    )
    ...
    cover_image = models.ImageField('Обложка', upload_to=book_cover_upload_path, blank=True, null=True)
...
class Work(models.Model):
    ...
    def get_work_type_group(self):
        ...
        groups = {
            ...
            'Поэзия': ['poem', 'verse', 'ballad', 'ode', 'elegy', 'sonnet', 'epigram', 'haiku', 'lyrics',
                       'epic_poem', 'rhyme'],
            ...
            'Научные': ['scientific_article', 'monograph', 'textbook', 'tutorial', 'methodical', 'thesis',
                        'abstract', 'lecture', 'research', 'dictionary_entry', 'encyclopedia', 'commentary'],
